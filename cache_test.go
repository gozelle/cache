...
func TestCacheTimes(t *testing.T) {
	...
	tc := New[int](WithDefaultExpiration(50*time.Millisecond), WithCleanupInterval(1*time.Millisecond))
	tc.Set("a", 1, DefaultExpiration)
...
func benchmarkCacheGet(b *testing.B, exp time.Duration) {
	b.StopTimer()
	tc := New[any](WithDefaultExpiration(exp))
	tc.Set("foo", "bar", DefaultExpiration)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tc.Get("foo")
...
func BenchmarkDeleteExpiredLoop(b *testing.B) {
	b.StopTimer()
	tc := New[any](WithDefaultExpiration(5 * time.Minute))
	tc.mu.Lock()
	for i := 0; i < 100000; i++ {
